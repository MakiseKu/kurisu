...
- 用户一直想开发牧濑红莉栖桌宠软件，并集成任务管理，番茄钟等等东西，然后整个交互要很活泼灵动很juicy，类似多邻国duolingo那样游戏化的体验，用游戏化的体验来自我提升。
- 但现在用户意识到或许方向错了，因为滴答清单已经把任务管理，番茄钟等等东西做的很好了。或许应该开发好桌宠的AI功能，让她直接给我建议，帮助用户进行自我提升。比如让她有"进程感知"功能（知道用户在用什么软件，软件内页面的名字：比如浏览器中正在浏览的网页的名字，vscode中正在操作的文件的名字）；有真正的"屏幕内容感知"功能：知道用户具体在做什么。比如在浏览器里的时候知道具体是什么网页，网页里有什么内容。比如在写代码时，能看到屏幕上写的是什么代码。
- 用户最开始是用Python的Flet库开发的番茄钟悬浮球软件，为此学习了大量的Python知识。我基本上能看懂我的项目里的每一行代码。后来，用户意识到用Python的Flet库做游戏化的软件太勉强了。于是用户试图拿Godot或者Java开发。学习了Godot和Java一段时间后，用户突然意识到，桌宠软件好像对性能的要求不高，或许应该换成Javascript或者typescript等等前端技术。前端更好写UI，而且实际上是网页，所以还是可以做到丰富的动效与游戏化的体验。 而且AI 对 Javascript或者typescript 相比Java和Godot更熟悉，可以帮助用户学习前端代码，甚至直接帮用户生成大量有效代码。而且先基于网页开发可以立刻看见效果，不需要等编译，等大致开发完毕后再打包成Electron。
